...
const docx = await exportLessonDocument(structuredClone(lessonExportFixture), 'docx');
assert.equal(docx.mimeType, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
assert.equal(docx.filename, '勾股定理探究课教学设计.docx');
...
const pdf = await exportLessonDocument(structuredClone(lessonExportFixture), 'pdf', {
...
assert.equal(pdf.filename, '勾股定理探究课教学设计.pdf');
