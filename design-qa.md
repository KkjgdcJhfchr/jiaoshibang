...
- 地址：登录后从“我的教案”打开当前账号实际生成的教案（路由为 `/app/lesson/<服务器教案编号>`）。
...
- 未填写环节时间、习题难度或建议时长时保持为空，不再伪造“0—0 分钟”“难度 0/5”或“建议 0 分钟”。
- 第三方数据以 `{ label: 'A', content: '…' }` 形式传入选项时只显示一个 A/B 标签，不再出现“A. A …”。
- 导出模型与 Word 生成器双层过滤 XML 1.0 禁止码点及孤立代理项，合法中文、换行和 Emoji 保持不变，避免生成损坏的 DOCX。
- 专项回归覆盖空数值、对象选项、非法字符、全部 XML/RELS 条目码点以及正常样例版式不回退。
