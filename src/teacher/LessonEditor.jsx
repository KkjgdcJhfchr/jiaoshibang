...
export function LessonEditor({ path }) {
  ...
  const sourceItems = isDemo
    ? [{ name: '教材《春》原文', detail: '图片 · 6 页 · 演示素材', mark: '春' }, { name: '七年级语文课程标准', detail: 'PDF · 演示素材', mark: '标' }]
  ...
  async function revise(instruction = feedback) {
    ...
    try {
      ...
      setToast(`修改已完成，并保存为本地版本 v${nextVersion}.0`);
    } catch (error) {
  ...
  async function submitTrainingCandidate(actionLabel) {
    if (isDemo) return;
    if (candidateSubmitted.current) return;
    const lessonConsent = localStorage.getItem('current-lesson-training-consent');
    if (lessonConsent === 'false') return;
    let hasConsent = lessonConsent === 'true';
    if (lessonConsent === null) {
      try {
        const session = await api.authSession();
        hasConsent = session.data?.user?.trainingConsent === true;
      } catch {
        hasConsent = false;
      }
    }
  ...
  return (
    <div className="editor-shell">
      <header className="editor-topbar">
        ...
        <div className="save-state">{isDemo ? <><CheckCircle2 size={16} /> 演示教案 · 修改仅保留在当前页面</> : dirty ? <><span className="unsaved-dot" /> 有未保存修改</> : <><CheckCircle2 size={16} /> 已保存到当前浏览器</>}</div>
      ...
      <div className="editor-layout">
        ...
        <main className="document-workspace">
          <article className="lesson-document">
            ...
            <footer className="document-footer"><span>{totalMinutes} 分钟课堂流程 · {exerciseCount} 道习题</span><span>{isDemo ? '演示内容' : `最后保存：${lesson.updated_at || '刚刚'}`}</span></footer>
          </article>
        </main>
        ...
        <aside className="ai-panel">
          ...
          <div className="ai-composer"><textarea value={feedback} onChange={(event) => setFeedback(event.target.value)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') revise(); }} placeholder="说说你想修改的地方…" maxLength={800} /><div><button title="当前修改接口暂不支持附加文件，请在左侧素材来源中添加" disabled><Paperclip size={17} /></button><span>{feedback.length}/800</span><Button size="sm" icon={Send} onClick={() => revise()} disabled={!feedback.trim() || revising}>发送</Button></div>{revisionError ? <p>{revisionError}</p> : null}</div>
          <small className="ai-disclaimer">内容由 AI 生成，请结合实际学情审核后使用</small>
        </aside>
      </div>

      <Modal open={versionsOpen} onClose={() => setVersionsOpen(false)} title="版本历史" description="当前编辑会话最多保留 20 个本地快照；刷新页面后不会继续保留这些快照。">
        <div className="version-list"><button className="current" type="button" disabled><span>v{currentVersion}.0</span><div><b>当前版本</b><p>{dirty ? '包含正在编辑、尚未失焦保存的内容' : isDemo ? '当前页面中的演示版本' : '已保存到当前浏览器'}</p><small>刚刚 · 当前教师</small></div><CheckCircle2 size={18} /></button>{[...historyRef.current].reverse().map((snapshot, index) => <button type="button" key={`${historyVersion}-${index}`} onClick={() => restoreSnapshot(snapshot)}><span>v{Math.max(1, currentVersion - index - 1)}.0</span><div><b>历史版本</b><p>{snapshot.metadata?.title || snapshot.metadata?.chapter || '教案快照'}</p><small>本次会话 · 可恢复</small></div><RotateCcw size={17} /></button>)}{!historyRef.current.length ? <p className="version-empty">当前会话还没有历史修改；完成编辑或 AI 修改后会自动生成快照。</p> : null}</div>
      </Modal>

      <Modal open={exportOpen} onClose={() => setExportOpen(false)} title="导出教案" description={`导出当前本地版本 v${currentVersion}.0。`} footer={<><Button variant="ghost" onClick={() => setExportOpen(false)}>取消</Button><Button icon={FileDown} onClick={exportDoc}>导出 DOC</Button></>}>
        <div className="export-options"><button className="selected" onClick={exportDoc}><FileText size={22} /><div><b>Word 文档</b><p>可继续编辑，包含完整教案和答案解析</p></div><CheckCircle2 size={18} /></button><button onClick={exportPrint}><Printer size={22} /><div><b>打印 / PDF</b><p>使用浏览器打印，可保存为 PDF 文件</p></div></button><button onClick={exportJson}><FileJson size={22} /><div><b>结构化 JSON</b><p>适合归档、二次开发或导入其他系统</p></div></button></div><label className="export-check" title="分页模板尚未接入"><input type="checkbox" disabled /> 习题答案另起一页（待接入）</label><label className="export-check" title="教师资料字段尚未接入"><input type="checkbox" disabled /> 封面显示学校和教师姓名（待接入）</label>
      </Modal>
