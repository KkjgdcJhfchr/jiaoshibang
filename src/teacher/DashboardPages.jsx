    ...
    <TeacherShell path={path} title={`你好，${displayName}`} subtitle="今天也一起备好一堂课。">
      <div className="dashboard-layout">
        ...
        <aside className="dashboard-rail">
          ...
          <section className="membership-card"><div className="rail-card-title"><b>当前账户</b><Crown size={18} /></div><h3><BadgeCheck size={18} /> 免费体验账户</h3><p>剩余 {credits} 次完整生成额度</p><ul><li><Check size={15} /> 教案生成与 AI 修改</li><li><Check size={15} /> DOCX / PDF 导出</li><li><Check size={15} /> 点数明细与套餐权益</li></ul><Button variant="secondary" onClick={() => navigate('/app/membership')}>查看套餐说明</Button></section>
        </aside>
      </div>
    </TeacherShell>
  );
  ...
  return (
    <TeacherShell path={path} title="会员中心" subtitle="查看会员权益并选择适合的套餐。">
      ...
      <div className="member-plan-grid">
        <article className={!currentMembership ? 'active' : ''}><header><h3>{freePlan?.name || '免费版'}</h3>{!currentMembership ? <span>当前账户</span> : null}</header><div className="member-price"><b>¥0</b><span>长期</span></div><p>{freePlan ? `注册赠送 ${freePlan.credits} 次教案生成点数` : '注册赠送体验点数'}</p><ul>{(freePlan?.features?.length ? freePlan.features : ['基础教案生成', 'AI 教案修改', 'DOCX / PDF 导出', '点数余额查询']).map((item) => <li key={item}><Check size={15} />{item}</li>)}</ul><Button variant="secondary" disabled>{!currentMembership ? '当前账户' : '基础权益保留'}</Button></article>
        {paidPlans.map((plan) => {
